...
const ProductDetails = () => {
    ...
    useEffect(() => {
        if(product && products.length > 0){
            const productCopy = products.filter((item)=> product.category === item.category && item._id !== product._id)
                ...
                <div className="flex gap-3">
                    <div className="flex flex-col gap-3">
                        {product?.images?.map((img, index) => (
                            <div 
                            key={index} 
                            onClick={() => setThumbnail(img)} className="border max-w-24 border-gray-500/30 rounded overflow-hidden cursor-pointer" >
